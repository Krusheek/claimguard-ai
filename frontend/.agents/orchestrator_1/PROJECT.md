...
|---|---------|-------------|-----------|--------|
| 1 | Enterprise Design Tokens & Theme | Custom Tailwind palette (slate, teal, emerald, amber, rose), typography hierarchy, utility classes | M1 | Survey R1 |
| 2 | Unified TypeScript Data Contracts | Standardized interfaces for Claims, Bills, Policies, Rejections, RuleVerdicts, Forensics, AuditTrail | M1 | Survey R1 |
| 3 | Shared Component Library & Skeletons | StatusBadge, MetricCard, realistic Skeletons, ErrorBoundary, EmptyState, Toast notifications | M1 | Survey R3 |
...
| 11 | Pre-Analysis Health Check & UX Transitions | Visual readiness checklist (Bill items, Policy sum insured, Rejection reasons) with smooth animation | M3 | Survey R3 |
...
|---|------|-------|-------------|--------|
| M1 | Foundations, Design Tokens & Shell | Tailwind theme, typography, types, API client normalizer, shared components, topbar/sidebar app shell | none | PLANNED |
| M2 | Enterprise Dashboard & Visualizations | Executive KPI cards, DashboardCharts (donut, waterfall, bar), advanced searchable/sortable claims table | M1 | PLANNED |
| M3 | Upload Studio & UX Polish | Dual-mode batch/stepper upload, document preview cards, pre-analysis check, loading/error states | M1 | PLANNED |
| M4 | Analysis & Forensics Hub | 4-tab workspace: Financial reconciliation & Rule verdicts, Forensics Lab (ELA/CGHS), Audit Trail, Grievance Letter | M1 | PLANNED |
...
## Interface Contracts
### Foundations (M1) -> Features (M2, M3, M4)
...
├── services/
│   ├── api.js                   # Normalized API client
│   └── mockData.js              # Realistic fallback data for demo / offline
├── components/
│   ├── common/                  # Shared UI components
│   │   ├── Topbar.jsx           # Global header with claim search, breadcrumbs, auditor profile
│   │   ├── StatusBadge.jsx      # Semantic status badges (PASS/FAIL/REVIEW/PENDING)
│   │   ├── MetricCard.jsx       # Financial KPI card with sparklines
│   │   ├── Skeletons.jsx        # Realistic skeleton screens
│   │   └── ErrorState.jsx       # Contextual error card with retry
│   ├── dashboard/
│   │   ├── DashboardCharts.jsx  # SVG/Visual charts (Status Donut, Financial Waterfall, Rule Bar)
│   │   └── ClaimsTable.jsx      # Searchable, sortable, paginated data table
│   ├── upload/
│   │   ├── BatchDropzone.jsx    # Smart multi-file drag-and-drop
│   │   ├── DocumentCard.jsx     # Uploaded file preview card
│   │   └── ReadinessCheck.jsx   # Pre-analysis verification checklist
│   └── analysis/
│       ├── FinancialDelta.jsx   # Financial reconciliation waterfall
│       ├── VerdictCard.jsx      # Rule verdict with slider & IRDAI citation
│       ├── ForensicsLab.jsx     # ELA tamper meter, heatmap viewer, CGHS benchmark
│       ├── AuditTimeline.jsx    # Cryptographic SHA-256 block ledger
│       └── AppealLetter.jsx     # Legal letterhead appeal generator
└── pages/
    ├── Dashboard.jsx
    ├── Upload.jsx
    └── Analysis.jsx
